...
case class Numbers(chromosomesAmount: Int, lineBitsAmount: Int):
  val numberLinesAmount = lineBitsAmount / longBitsAmount + (if lineBitsAmount % longBitsAmount > 0 then 1 else 0)
  ...
  def swapGenerations() =
    if currentGenPos == 0 then currentGenPos = generationNumbersAmount
  ...
  override def toString: String =
    ...
    while p >= 0 do
      ...
      p -= 1
      if splitByChromosomePos == chromosomeNumbersAmount - 1 then
        splitByChromosomePos = 0
        str += "\n\n"
      else splitByChromosomePos += 1
      if splitByLinePos == numberLinesAmount - 1 then
        splitByLinePos = 0
        str += "\n"
      else splitByLinePos += 1
      if divideBy2Pos == numbers.length / 2 - 1 then
        divideBy2Pos = 0
        str += "\n"
        insertSplitLine
        str += "\n\n\n"
