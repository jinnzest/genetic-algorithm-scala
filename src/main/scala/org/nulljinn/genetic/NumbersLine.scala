...
case class NumbersLine(numbers: Array[Long], size: Int):
  ...
  def crossBits(from: NumbersLine, fromBit: Int, bitsAmount: Int, bidirectional: Boolean): Unit =
    ...
    val firstBitsAmount = if bitsLeft >= nBitsAmount then nBitsAmount else bitsLeft
    ...
    val fullBytesFrom = if shiftFrom > 0 then pByteFrom + 1 else pByteFrom

    val pByteTo = (afterLastBit - 1) / longBitsAmount

    val fullBytesTo = if shiftTo != 0 then pByteTo - 1 else pByteTo

    val fullBytesPresent = (nBitsAmount + shiftFrom) / longBitsAmount > 0 || shiftFrom == 0 && nBitsAmount / longBitsAmount > 0

    val fromPresent = if fullBytesPresent && shiftFrom == 0 then false else shiftTo > 0 || shiftFrom > 0
